...
def test_Check_structure(browser):
    ...
    assert browser.find_element_by_xpath("//button[@aria-label='Toggle PHASE57']/..").get_attribute("aria-expanded") == 'true'
    assert browser.find_element_by_xpath("//button[@aria-label='Toggle PHASE57']/../div[2]").get_attribute("class") == ''
    before_value = browser.find_element_by_xpath("//span[contains(text(),'TEMP')]/../mat-form-field/div/div/div/input").get_attribute("value")
    ...
    time.sleep(2)
    browser.find_element_by_xpath("/html/body/app-root/div").click()
    after_value = browser.find_element_by_xpath("//span[contains(text(),'TEMP')]/../mat-form-field/div/div/div/input").get_attribute("value")
    ...
    phase_ele = browser.find_element_by_xpath("//button[@aria-label='Toggle PHASE57']")
    MainPage(browser).eleclick(phase_ele)
    assert browser.find_element_by_xpath("//button[@aria-label='Toggle PHASE57']/..").get_attribute("aria-expanded") == 'false'
    assert browser.find_element_by_xpath("//button[@aria-label='Toggle PHASE57']/../div[2]").get_attribute("class") == 'example-tree-invisible'
    # Click "+" icon to add elements
    browser.find_element_by_xpath("//button[@aria-label='Toggle PHASE51']").click()
    time.sleep(3)
    browser.find_element_by_xpath("//button[@aria-label='Toggle PHASE51']/../div[2]/cdk-nested-tree-node[1]/button").click()
    time.sleep(3)
    Yes_color = browser.find_element_by_xpath("//input[@aria-checked='true']/..").value_of_css_property('background-color')
    No_color = browser.find_element_by_xpath("//input[@aria-checked='false']/..").value_of_css_property('background-color')
    # Check the grey stands for No, the blue stands for Yes
    assert Yes_color == 'rgba(38, 152, 251, 0.54)'
    assert No_color == 'rgba(0, 0, 0, 0.38)'
    Valuelist = browser.find_elements_by_xpath("//div[@class='mat-slide-toggle-thumb']/../../../../../../span")
    browser.find_element_by_xpath("//span/mat-icon[text()='add']").click()
    time.sleep(3)
    # click cancel add
    ...
    cancel_add = browser.find_elements_by_xpath("//div[@class='mat-slide-toggle-thumb']/../../../../../../span")
    # click add OK
    browser.find_element_by_xpath("//span/mat-icon[text()='add']").click()
    time.sleep(3)
    browser.find_element_by_xpath("//*[@id='error-message-dialog']/div/div[3]/button[2]").click()
    time.sleep(5)
    Valuelist_add = browser.find_elements_by_xpath("//div[@class='mat-slide-toggle-thumb']/../../../../../../span")
    assert len(Valuelist) == len(cancel_add)
    assert len(Valuelist_add) == len(Valuelist)+1
    # Click "-" icon to delete elements
    MainPage(browser).eleclick(Valuelist_add[2])
    browser.find_element_by_xpath("//span/mat-icon[text()='remove']").click()
    time.sleep(3)
    # click cancel remove
    browser.find_element_by_xpath("//*[@id='error-message-dialog']/div/div[3]/button[1]").click()
    time.sleep(2)
    cancel_remove = browser.find_elements_by_xpath("//div[@class='mat-slide-toggle-thumb']/../../../../../../span")
    # click remove ok
    ...
    Valuelist_remove = browser.find_elements_by_xpath("//div[@class='mat-slide-toggle-thumb']/../../../../../../span")
    assert len(Valuelist_add) == len(cancel_remove)
    assert len(Valuelist_remove) == len(Valuelist_add) - 1
    # for array type
    phasename_list = browser.find_elements_by_xpath("//cdk-tree/cdk-nested-tree-node/div[1]/div")
    phasenames = []
    ...
    phase_list = browser.find_elements_by_xpath("//div[@class = 'phase-name-text']")
    phase_names = []
